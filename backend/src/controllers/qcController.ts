...
export const getQCChecksByBatch = async (req: Request, res: Response) => {
  try {
    const { batchId } = req.params;
    const filters: QCFilters = {
      batchId,
      page: 1,
      limit: 100 // Get all checks for a batch
    };
    const result = await qcService.getAllQCChecks(filters);
    return successResponse(res, result.checks);
  } catch (error: any) {
    return errorResponse(res, error.message);
  }
};
...
export const getQCStats = async (req: Request, res: Response) => {
  try {
      ...
      const stats = await qcService.getQCStats(dateRange);
...
export const getDefectAnalysis = async (req: Request, res: Response) => {
    try {
        ...
        const dateRange = startDate && endDate ? { start: startDate, end: endDate } : undefined;
        const defects = await qcService.getDefectsByType(dateRange);
