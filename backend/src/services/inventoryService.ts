...
export class InventoryService {
  ...
  async getAllItems(filters: InventoryFilters) {
    ...
    if (search) {
      query = query.where(builder => {
        builder.where('inventory_items.name', 'ilike', `%${search}%`)
          .orWhere('inventory_items.code', 'ilike', `%${search}%`);
      });
    }
    ...
    if (lowStock) {
      query = query.whereRaw('inventory_items.quantity <= inventory_items.reorder_level');
    }

    // Get total count for pagination
    const countQuery = query.clone().clearSelect().count('inventory_items.id as total').first();
    const totalResult = await countQuery;
    const total = parseInt(totalResult?.total as string || '0');

    // Get data
    const items = await query.orderBy('inventory_items.name', 'asc').limit(limit).offset(offset);
  ...
  async searchItems(term: string) {
      return db('inventory_items')
        .leftJoin('suppliers', 'inventory_items.supplier_id', 'suppliers.id')
        .where('inventory_items.name', 'ilike', `%${term}%`)
        .orWhere('inventory_items.code', 'ilike', `%${term}%`)
        .select(
          'inventory_items.*',
          'suppliers.name as supplier_name'
        )
